...
// InitializeRedis 连接 redis 数据库，设置全局的 Redis 对象
...
func (rds RedisClient) Set(key string, value interface{}, expiration time.Duration) bool {
	if err := rds.Client.Set(rds.Context, key, value, expiration).Err(); err != nil {
		logger.Logger().Error("Redis", "Set", err.Error())
		return false
	}
	return true
}
...
func (rds RedisClient) Get(key string) string {
	...
	if err != nil {
		if err != redis.Nil {
			logger.Logger().Error("Redis", "Get", err.Error())
		}
		return ""
	}
	return result
...
func (rds RedisClient) Has(key string) bool {
	...
	if err != nil {
		if err != redis.Nil {
			logger.Logger().Error("Redis", "Has", err.Error())
		}
		return false
	}
	return true
...
func (rds RedisClient) Del(keys ...string) bool {
	if err := rds.Client.Del(rds.Context, keys...).Err(); err != nil {
		logger.Logger().Error("Redis", "Del", err.Error())
		return false
	}
	return true
}
...
func (rds RedisClient) FlushDB() bool {
	if err := rds.Client.FlushDB(rds.Context).Err(); err != nil {
		logger.Logger().Error("Redis", "FlushDB", err.Error())
		return false
	}
	return true
}
...
func (rds RedisClient) Increment(parameters ...interface{}) bool {
	...
	case 1:
		...
		if err := rds.Client.Incr(rds.Context, key).Err(); err != nil {
			logger.Logger().Error("Redis", "Increment", err.Error())
			return false
		}
	case 2:
		key := parameters[0].(string)
	...
	default:
		logger.Logger().Error("Redis", "Increment", "参数过多")
		return false
	}
	return true
}
...
func (rds RedisClient) Decrement(parameters ...interface{}) bool {
	...
	case 1:
		...
		if err := rds.Client.Decr(rds.Context, key).Err(); err != nil {
			logger.Logger().Error("Redis", "Decrement", err.Error())
			return false
		}
	case 2:
		key := parameters[0].(string)
	...
	default:
		logger.Logger().Error("Redis", "Decrement", "参数过多")
		return false
	}
	return true
}
